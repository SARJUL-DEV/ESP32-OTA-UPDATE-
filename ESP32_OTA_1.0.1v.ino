...
String buildPageHTML(const IPAddress &ip) {
  ...
  size_t maxFirmware = ESP.getFreeSketchSpace(); // bytes available for new firmware
...
function upload(){
  ...
  xhr.onload = function(){
    ...
    if(resp.trim() == 'FLASH_OK'){
      ...
      bar.style.width = '100%';
      // Auto-refresh after short delay to show updated storage/status
...
void setup(){
  ...
  server.begin();
  Serial.println("AP started. Connect to SSID: Sarjul");
}
